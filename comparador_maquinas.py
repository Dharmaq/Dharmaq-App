...
st.set_page_config(page_title="Simulador Dharmaq - In.Pack", layout="wide")
...
st.markdown("### Comparativo: Máquina do Cliente x In.Pack x Cenário de Equiparação")

# =========================================================
# DADOS BASE - IGUAIS À PLANILHA Pasta10
# =========================================================
dados_base = {
    "Cliente": {
...
# =========================================================
# FUNÇÃO DE CÁLCULO - IDÊNTICA À PLANILHA Pasta10
# =========================================================
def calcular(m):
...
# =========================================================
# SIDEBAR - EDITAR DADOS
# =========================================================
st.sidebar.title("Parâmetros das Máquinas")
...
nomes = [maquinas[k]["nome"] for k in keys]
cores = [cores_map[k] for k in keys]
...
keys_vis = [k for k in keys if mostrar[k]]

if not keys_vis:
    st.warning("Selecione ao menos um cenário para exibir.")
    st.stop()

nomes_vis = [maquinas[k]["nome"] for k in keys_vis]
cores_vis  = [cores_map[k] for k in keys_vis]
...
for col, k in zip(card_cols, keys_vis):
    ...
    kg_mes = r[k]["kg_mes"]
    conv_k = r[k]["conv_kg"]
    payback = maquinas[k]["exw"] * (1 + maquinas[k]["cif_pct"] / 100) / lucro if lucro > 0 else 0
    ...
    with col:
        ...
        <div style='background-color:#1e2130; padding:20px; border-radius:12px;
            ...
            <h2 style='color:{cor}; margin:0'>{sinal}€ {lucro:,.2f}</h2>
            ...
            <h3 style='color:{"#22c55e" if ganho_vs_atual >= 0 else "#ef4444"}; margin:0'>
                {sinal_g}€ {ganho_vs_atual:,.2f}
            </h3>
            <p style='color:#aaa; margin:8px 0 2px'>Custo conversão / kg</p>
            <h3 style='color:white; margin:0'>€ {conv_k:.6f}</h3>
            <p style='color:#aaa; margin:8px 0 2px'>Payback estimado</p>
            <h3 style='color:white; margin:0'>{payback:.2f} anos</h3>
        </div>
        """, unsafe_allow_html=True)

# =========================================================
# SEÇÃO 2 - GAUGE DE PRODUÇÃO
# =========================================================
...
for i, (k, cor) in enumerate(zip(keys_vis, cores_vis)):
...
# =========================================================
# SEÇÃO 4 - WATERFALL (onde vai o dinheiro)
# =========================================================
st.markdown("## Onde vai o dinheiro? (Faturamento anual)")
st.caption("Como o faturamento se divide entre custo de material, custo de conversão e lucro líquido")

wf_cols = st.columns(len(keys_vis))
for col, k in zip(wf_cols, keys_vis):
    cor   = cores_map[k]
    fat   = r[k]["fatur_anual"]
    mat   = r[k]["custo_material_ano"]
    conv  = r[k]["conv_ano"]
    lucro = r[k]["lucro_liq"]
    ...
    fig_wf = go.Figure(go.Waterfall(
        ...
        x=["Faturamento", "- Custo Bobina", "- Custo Conversão", "= Lucro Líq."],
        y=[fat, -mat, -conv, lucro],
        text=[f"€{fat:,.0f}", f"-€{mat:,.0f}", f"-€{conv:,.0f}", f"€{lucro:,.0f}"],
        textposition="outside",
        connector={"line": {"color": "#555"}},
        increasing={"marker": {"color": cor}},
        decreasing={"marker": {"color": "#ef4444"}},
        totals={"marker": {"color": "#22c55e" if lucro >= 0 else "#ef4444"}},
    ))
    fig_wf.update_layout(
        title=dict(text=maquinas[k]["nome"], font=dict(color=cor)),
        paper_bgcolor="#0e1117", plot_bgcolor="#1e2130",
        font_color="white", height=420,
        margin=dict(t=60, b=20, l=20, r=20),
        showlegend=False,
    )
    fig_wf.update_yaxes(gridcolor="#333")
    col.plotly_chart(fig_wf, use_container_width=True)

# =========================================================
# SEÇÃO 5 - FLUXO DE CAIXA ACUMULADO (com investimento inicial)
# =========================================================
...
# =========================================================
# SEÇÃO 6 - PIZZA DE CUSTOS MENSAIS
# =========================================================
...
# =========================================================
# SEÇÃO 7 - TABELA COMPLETA (IGUAL À PLANILHA)
# =========================================================
